...
export default class ServicesWorkflow extends Workflow {
   ...
   * - {detachInformations}: Object that contains detach information
   * @param {Object} DetachService   Service to handle request to perform a
   * detach action, see /services API schema
   */
  constructor($translate, workflowOptions, DetachService) {
    ...
    if (!this.serviceId) {
      throw new Error(
        'WucProductOffers - ServicesWorkflow: serviceId is undefined',
      );
    }
  }
  ...
  getPricings() {
    ...
    const getDetachPlancodePromise =
      this.detachInformation !== undefined
        ? Promise.resolve([this.detachInformation])
        : this.workflowService.getAvailableDetachPlancodes(this.serviceId);

    return getDetachPlancodePromise
      .then((plancodes) => {
        const [detachPlancode] = plancodes; // Yes, for the moment, there is just one plan code
        this.plancode = detachPlancode.planCode;
        this.pricings = detachPlancode.pricings.map((pricing) =>
          Workflow.transformPricing(pricing),
        );
